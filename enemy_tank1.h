class CEnemyTank01:public CEnemy{
public:
	void UpdateEnemy();
	void CreateEnemy(){
		mLandscapeFlg=true;
		...
		for(int i=0;i<4;i++){
			mBackParts[i  ].InitParts(ENEMY_PARTS_TANK1_MERUS_LEFT , 4,24+i*10);
			mBackParts[i+4].InitParts(ENEMY_PARTS_TANK1_MERUS_RIGHT,46,24+i*10);

			mBackParts[i+ 8].InitParts(ENEMY_PARTS_TANK1_PROPODUS_LEFT);
			mBackParts[i+12].InitParts(ENEMY_PARTS_TANK1_PROPODUS_RIGHT);
		}
		mAutoDelFlg=false;
		if(m_y0<380){
...
void CEnemyTank01::UpdateEnemy(){
	...
	for(int i=0;i<4;i++){
		int i01=0;
		if(i%2==0)i01=1;
		mBackParts[i  ].SetAngle(-50+i*20+(int)(40*sin(N_PI*(omega*mTimer+i01*90))));
		mBackParts[i+4].SetAngle( 50-i*20+(int)(40*sin(N_PI*(omega*mTimer+i01*90))));

		LinkBackParts(i+8,i,-10+i*20+(int)(60*sin(N_PI*(omega*mTimer+i01*90))));
		LinkBackParts(i+12,i+4, 10-i*20+(int)(60*sin(N_PI*(omega*mTimer+i01*90))));
	}

}

void CEnemyTank01::Killed(){
	m_y += 4.0;
	/////////////        PURGE  PARTS      ///////////////////
	if (mKilledTimer == 1) {
		for (int i = 0; i < mMaxParts; i++)
		{
			    mParts[i].SetVel((double)7 * (rand() % mMaxParts     -     mMaxParts / 2), (double)2 * (i + 1));
		}
		for (int i = 0; i < mMaxBackParts; i++)
		{
			mBackParts[i].SetVel((double)8 * (rand() % mMaxBackParts - mMaxBackParts / 2), (double)3 * (i + 1));
		}
	}
	for (int i = 0; i < mMaxParts; i++) {
