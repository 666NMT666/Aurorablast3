class CEnemyFO01:public CEnemy{
protected:
public:
	void UpdateEnemy();
	void Killed();
...
void CEnemyFO01::UpdateEnemy(){
	if(m_y>300+100*mGameLevel)mDisabledFlg=true;
	...
	if(rand()%freq==0){
		...
		if(abs(m_x-mPlayer->GetX())>distanceLimit[mGameLevel] || abs(m_y-mPlayer->GetY())>distanceLimit[mGameLevel]){
			for(int i=0;i<2;i++)NormalShot((int)m_x, (int)m_y,EB_26,2,14-2*i-(5-mGameLevel));
		}
	}
	BackLightSmall();
}

void CEnemyFO01::Killed(){
