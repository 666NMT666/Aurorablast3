#pragma once
class CEnemyLarge02 :public CEnemy {
public:
	void UpdateEnemy();
	void Killed();
};

void CEnemyLarge02::UpdateEnemy() {
	if (mTimer <= 1) {
		mAutoDelFlg = false;
		m_vx = 0; m_vy = 0;
		mInvincibleFlg = true;
	...
	if (mTimer>60) {
		int freq[5] = { 12,11,11,10,10 };
		if (mTimer%freq[mGameLevel] == 0) {
			mSE->PlaySingleSound(SE_SHT4);
			int p[5] = { EMISSILE_STOP_GO,10,20,1,0 };
			CCreateInfo info(p);
			mEMManager->Create<CEnemyMissile>((int)m_x - 60, (int)m_y, 0, 0, 0, -20 - rand() % (20 + mGameLevel * 5), info);
			mEMManager->Create<CEnemyMissile>((int)m_x + 60, (int)m_y, 0, 0, 0, -20 - rand() % (20 + mGameLevel * 5), info);
		}
	}
}

void CEnemyLarge02::Killed() {
	m_y += 2;
	m_x += rand() % 3 - 1;
