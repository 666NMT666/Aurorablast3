class CEnemyShield2:public CEnemy{
private:
	CVector mTarget;
	RECT mRectShield;
public:
	CEnemyShield2():CEnemy(){mTarget=CVector::Vector(0.0,0.0);}
...
void CEnemyShield2::UpdateEnemy(){
	...
	if(startTime<mTimer && mCounter==5 && mTimer%3==0){
		mCounter++;
		mSE->PlaySingleSound(SE_EXP5);
		CCreateInfo info=defaultCreateInfo;
		AngleMissile((int)m_x-rand()%40,(int)m_y,EB_MISSILE_100x20,0,18.0,90,CCreateInfo::CCreateInfo(EMISSILE_SHOT,90,0,0,0));
	}
	BackLightSmall(m_x,m_y-10);
	if(mTimer>startTime+50)m_vy-=0.25;
}
void CEnemyShield2::UpdateEnemyPre(){
	if(mBulletManager->HitTestRectWithKind(mRectShield,0,m_x,m_y)){
		mSE->PlayLoopSound(SE_SLD1);
	}
}
void CEnemyShield2::Killed(){
