...
class CEnemyAmblypygi2 : public CEnemy {
public:
	void UpdateEnemy();
	void CreateEnemy() {
		mLandscapeFlg = true;
		mKilledSound = SE_EXP3;
		mParts[0].InitParts(COMMON_PARTS_CANNON30x79, m_width / 2, m_height / 2);//

		mParts[1].InitParts(ENEMY_PARTS_WS2_WHIP1, m_width / 2 - 16, m_height / 2 + 28);
		mParts[2].InitParts(ENEMY_PARTS_WS2_WHIP1, m_width / 2 + 16, m_height / 2 + 28);
		mParts[3].InitParts(ENEMY_PARTS_WS2_WHIP2);
		mParts[4].InitParts(ENEMY_PARTS_WS2_WHIP2);
		mParts[5].InitParts(ENEMY_PARTS_WS2_WHIP3);
		mParts[6].InitParts(ENEMY_PARTS_WS2_WHIP3);
		mParts[7].InitParts(ENEMY_PARTS_WS2_PEDIPALP1_LEFT, m_width / 2 - 10, m_height / 2 + 27);
		mParts[8].InitParts(ENEMY_PARTS_WS2_PEDIPALP1_RIGHT, m_width / 2 + 10, m_height / 2 + 27);
		mParts[9].InitParts(ENEMY_PARTS_WS2_PEDIPALP2_LEFT);
		mParts[10].InitParts(ENEMY_PARTS_WS2_PEDIPALP2_RIGHT);

		for (int i = 0; i<3; i++) {
			mBackParts[i].InitParts(ENEMY_PARTS_WS2_MERUS_LEFT, 2, 47 + i * 7);
			mBackParts[i + 3].InitParts(ENEMY_PARTS_WS2_MERUS_RIGHT, m_width - 3, 47 + i * 7);

			mBackParts[i + 6].InitParts(ENEMY_PARTS_WS2_PROPODUS_LEFT);
			mBackParts[i + 9].InitParts(ENEMY_PARTS_WS2_PROPODUS_RIGHT);

			mBackParts[i + 12].InitParts(ENEMY_PARTS_WS2_DACTYLUS_LEFT);
			mBackParts[i + 15].InitParts(ENEMY_PARTS_WS2_DACTYLUS_RIGHT);
		...
		CExRect::InitRect(&mRectBulletHit, -m_width / 2 - 10, -m_height / 2, m_width / 2 + 10, m_height / 2 + 10);
	}
	void Killed();
	void Walk();
};

void CEnemyAmblypygi2::Walk() {
	int omega = 33;
	...
	for (int i = 0; i<NUM_LEG_PARES; i++) {
		...
		LinkBackParts(i + NUM_LEG_PARES * 2, i + NUM_LEG_PARES * 0, 80 + i * 17 - (int)(10 * sin(N_PI*(omega*mTimer + i01 * 90))));
		LinkBackParts(i + NUM_LEG_PARES * 3, i + NUM_LEG_PARES * 1, -80 - i * 17 - (int)(10 * sin(N_PI*(omega*mTimer + i01 * 90))));
		LinkBackParts(i + NUM_LEG_PARES * 4, i + NUM_LEG_PARES * 2, 40 + i * 27 - (int)(40 * sin(N_PI*(omega*mTimer + i01 * 90))));
		LinkBackParts(i + NUM_LEG_PARES * 5, i + NUM_LEG_PARES * 3, -40 - i * 27 - (int)(40 * sin(N_PI*(omega*mTimer + i01 * 90))));
	}
...
void CEnemyAmblypygi2::UpdateEnemy() {
	Walk();
	int angle = 0;
	if (m_y0<380) {
	...
	if (mTimer<150) {
		if (mTimer % 20 == 0)m_vx = 0.1*(rand() % 120 - 60);
