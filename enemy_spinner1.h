class CEnemySpinner1:public CEnemy{
public:
	void UpdateEnemy();
	void Killed();
};

void CEnemySpinner1::UpdateEnemy(){
	...
	if(mTimer>100-mGameLevel*4){
		if(mTimer%3==0 && mTimer%20<10){
			mSE->PlaySingleSound(SE_EXP2);
			NormalShot(m_x,m_y,EB_40,2,6+rand()%5,2,rand()%13-6);
		}
	}
}

void CEnemySpinner1::Killed(){
