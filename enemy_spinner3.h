class CEnemySpinner3:public CEnemy{
public:
	void UpdateEnemy();
	void Killed();
};

void CEnemySpinner3::UpdateEnemy(){
	...
	if(mTimer>100-mGameLevel*4){
		...
		CVector v = CVector::EigenVector(mPlayer->GetX() - m_x, mPlayer->GetY() - m_y);
		m_vx = v.x * 4.0;
		m_vy = v.y * 4.0;
	}
}

void CEnemySpinner3::Killed(){
